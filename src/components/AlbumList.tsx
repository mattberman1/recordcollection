          ...
          <div className="divide-y divide-gray-100">
            {sortedArtists.map((artist, artistIndex) => (
              <div key={artist} className="p-6">
                {/* Artist Header */}
                <div className="mb-4">
                  <h3 className="text-lg font-semibold text-gray-900 border-b border-gray-200 pb-2 text-left">
                    {artist}
                ...
                <div className="space-y-3">
                  {groupedAlbums[artist].map((album) => (
                    <div
                      key={album.id}
                      className="flex items-center justify-between space-x-4 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer group"
                    ...
                    >
                      <div className="flex items-center space-x-4 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0 flex flex-row items-center">
                          <h4 className="text-lg font-medium text-gray-900 truncate group-hover:text-blue-600 transition-colors text-left flex-1">
                            {album.title}
                          </h4>
                          <span className="text-sm text-gray-500 w-20 text-right tabular-nums flex-shrink-0">
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-xl shadow">
        ...
        <div className="divide-y divide-gray-100">
          {albums.map((album) => (
            <div
              key={album.id}
              className="flex items-center justify-between space-x-4 p-6 hover:bg-gray-50 transition-colors cursor-pointer group"
              onClick={() => handleAlbumClick(album)}
            >
              <div className="flex items-center space-x-4 flex-1 min-w-0">
                <div className="flex-shrink-0">
              ...
              <div className="flex items-center space-x-4">
                <span className="text-sm text-gray-500 text-right min-w-[100px]">
                  {album.artist}
                </span>
                <button
                  onClick={(e) => {
